  ...
  useEffect(() => {
    const layer = L.geoJSON(geojson, {
      ...
      onEachFeature: (feature: any, lyr: L.Layer) => {
        ...
        if (p.tipo_produto) tip += `<br>${TIPO_PRODUTO_LABELS[p.tipo_produto as keyof typeof TIPO_PRODUTO_LABELS] ?? p.tipo_produto}`
...
function SortablePedidoRow_Base({
  ...
  const handleItemDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      await onReorderItems(p, reordered.map((i) => i.id))
    } catch {
      setLocalItems([...p.itens].sort((a, b) => a.prioridade - b.prioridade))
    }
  }
    ...
    >
      ...
      {p.auto_submitted && (
        <div className="flex items-center gap-2 px-4 py-2 bg-amber-500/10 border-b border-amber-500/20 text-xs text-amber-300">
          ...
          Este pedido foi enviado automaticamente no fim do prazo de solicitações
        </div>
      )}
      ...
      <div className="flex items-center gap-1">
        ...
        >
          ...
          <span className="text-[10px] font-bold text-zinc-600 group-hover:text-zinc-400 w-3">{rank}</span>
        </button>
        ...
        >
          ...
          <div className="shrink-0 flex flex-col items-start gap-1 w-24">
            ...
            <span className={`inline-flex items-center text-[10px] font-medium px-1.5 py-0.5 rounded-full border ${statusSimpl.cls}`}>
              {statusSimpl.label}
            </span>
          </div>
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm text-zinc-100 font-medium truncate">{tipos}</p>
      ...
      {isExpanded && (
        <div className="border-t border-white/5 bg-zinc-950/30">
          ...
          <div className="px-5 pt-4 pb-3 border-b border-white/5">
            <p className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wide mb-2">Dados do solicitante</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-2 text-xs">
              <div className="flex items-center gap-1.5 text-zinc-300">
                <User className="h-3 w-3 text-zinc-500 shrink-0" />
                <span className="truncate">{p.usuario_nome ?? user?.nome ?? '—'}</span>
              </div>
              {user?.om && (
          ...
          <div className="px-5 py-3 grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-3 text-xs border-b border-white/5">
            <div>
              ...
              <span className={`inline-flex items-center text-[11px] font-medium px-2 py-0.5 rounded-full border ${statusSimpl.cls}`}>
                {statusSimpl.label}
              </span>
            </div>
            <div>
              ...
              <p className="text-zinc-300">
                {format(new Date(p.criado_em), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })}
              </p>
            </div>
            <div>
            ...
            {p.link_bdgex && (
              <div className="col-span-2 sm:col-span-3">
                <p className="text-zinc-500 mb-0.5">Dados disponíveis no BDGEx</p>
                <a
                  href={p.link_bdgex}
                  target="_blank"
                ...
                {localItems.map(item => (
                  ...
                  >
                    ...
                    {item.disponivel_bdgex && (
                      <span className="ml-auto text-emerald-500 font-medium">Disponível no BDGEx</span>
                    )}
                  </div>
                ))}
              </div>
...
const SortablePedidoRow = memo(SortablePedidoRow_Base, (prev, next) =>
...
function SortableItemRow({
  ...
  return (
    ...
    >
      ...
      <span className="text-zinc-500">{item.escala}</span>
      {item.disponivel_bdgex && (
        <span className="text-emerald-500 font-medium">Disponível no BDGEx</span>
      )}
      <button
        onClick={onDelete}
        disabled={!canDelete}
        ...
        title={canDelete ? 'Remover produto' : 'Não é possível remover o único produto'}
      >
        <Trash2 className="h-3.5 w-3.5" />
      </button>
    </div>
...
function EnviarPedidosModal({ rascunhos, onClose, onEnviado }: EnviarPedidosModalProps) {
  const { user } = useAuthStore()
  ...
  const handleEnviar = async () => {
    ...
    try {
      ...
      toast.success(`${res.data.length} pedido${res.data.length !== 1 ? 's' : ''} enviado${res.data.length !== 1 ? 's' : ''} com sucesso!`)
      onEnviado(res.data)
      onClose()
    } catch {
          ...
          <>
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-start gap-3 p-4 bg-amber-500/10 border border-amber-500/25 rounded-xl">
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-200">Tem certeza que deseja enviar todos os pedidos?</p>
                  <p className="text-xs text-amber-300/80 mt-1 leading-relaxed">
                    Após o envio não será possível realizar alterações. Os {rascunhos.length} pedido{rascunhos.length !== 1 ? 's' : ''} em rascunho serão encaminhados para análise.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-1 max-h-48 overflow-y-auto">
                {rascunhos.map((p, i) => {
                  const tipos = [...new Set(p.itens.map(it => TIPO_PRODUTO_LABELS[it.tipo_produto]))].join(', ')
                  return (
                    <div key={p.id} className="flex items-center gap-3 text-xs bg-zinc-800/50 rounded-lg px-3 py-2">
                      <span className="text-zinc-500 w-4 shrink-0">{i + 1}º</span>
                      <span className="font-mono text-emerald-400 shrink-0">#{p.id}</span>
                      <span className="text-zinc-300 truncate">{tipos}</span>
                      <span className="text-zinc-600 ml-auto shrink-0">{p.itens.length} prod.</span>
                    </div>
                  )
                })}
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-2 p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                ...
                <p className="text-xs text-emerald-300/80">
                  {rascunhos.length} pedido{rascunhos.length !== 1 ? 's' : ''} pronto{rascunhos.length !== 1 ? 's' : ''} para envio
                </p>
              </div>
            </div>
            <div className="flex gap-2 px-6 pb-6">
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-zinc-900 border border-white/10 rounded-2xl shadow-2xl w-full max-w-sm">
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <h2 className="text-base font-semibold text-zinc-100">Remover Pedido #{pedido.id}</h2>
          <button onClick={onClose} className="text-zinc-500 hover:text-zinc-300 transition-colors">
            <X className="h-5 w-5" />
          </button>
        </div>
        <div className="p-6">
          <p className="text-sm text-zinc-400">
            Tem certeza que deseja remover este rascunho? Esta ação não poderá ser desfeita.
          </p>
        </div>
        <div className="flex gap-2 p-6 pt-0">
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      ...
      <div className="bg-zinc-900 border border-white/10 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-5 border-b border-white/10 flex-shrink-0">
          <div>
            <h2 className="text-base font-semibold text-zinc-100">Pedido #{pedido.id} — Ver no mapa</h2>
            <p className="text-xs text-zinc-500 mt-0.5">{pedido.itens.length} produto{pedido.itens.length !== 1 ? 's' : ''}</p>
          </div>
          <button onClick={onClose} className="text-zinc-500 hover:text-zinc-300 transition-colors">
            <X className="h-5 w-5" />
          </button>
          ...
          <div className="flex flex-wrap gap-1.5">
            {pedido.itens.map(item => (
              ...
              >
                ...
                {item.mi && <><span className="text-zinc-600">·</span><span className="text-zinc-500">MI {item.mi}</span></>}
                <span className="text-zinc-600">·</span>
                <span className="text-zinc-400">{TIPO_PRODUTO_LABELS[item.tipo_produto]}</span>
              </span>
            ))}
...
export function MeusPedidos() {
  ...
  useEffect(() => {
    janelasApi.minhaJanela().then(r => setMinhaJanela(r.data)).catch(() => {})
  }, [])
  ...
  const dataFimJanela = minhaJanela?.data_fim ? new Date(minhaJanela.data_fim) : null
  const dataInicioJanela = minhaJanela?.data_inicio ? new Date(minhaJanela.data_inicio) : null
  const janelaFechada = minhaJanela?.configurada
    ? (!janelaAtiva && dataFimJanela ? isAfter(now, dataFimJanela) : false)
    : false
  ...
  const handlePedidoDragEnd = useCallback(async (event: DragEndEvent) => {
    const { active, over } = event
    if (!over || active.id === over.id) return
  ...
  const handleReorderItems = useCallback(async (pedido: Pedido, orderedIds: number[]) => {
    await pedidosApi.reorderItems(pedido.id, orderedIds)
  }, [])

  const handleDeleteItem = useCallback(async (pedidoId: number, itemId: number) => {
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-xl font-semibold text-zinc-100 tracking-tight">Resumo dos pedidos</h1>
        </div>
          ...
          <div className="px-6 py-5 text-center">
            <div className="flex justify-center mb-3">
              {janelaFechada
                ...
                : urgente
                  ...
                  : <CalendarClock className={`h-8 w-8 ${janelaAtiva ? 'text-emerald-400' : 'text-blue-400'}`} />
              }
            </div>
            <p className={`text-xl font-bold leading-snug ${
              janelaFechada ? 'text-zinc-400' : urgente ? 'text-red-200' : janelaAtiva ? 'text-zinc-100' : 'text-blue-200'
            }`}>
              {janelaFechada
            ...
            {janelaAtiva && todosEnviados && (
              <div className="mt-3 mx-auto max-w-sm bg-amber-500/10 border border-amber-500/25 rounded-xl px-4 py-3 text-left">
                <p className="text-xs font-semibold text-amber-300 mb-1">Pedidos já encaminhados</p>
                <p className="text-xs text-amber-200/70 leading-relaxed">
                  Caso haja a necessidade de alteração do pedido, entre em contato com a <strong>SSGeoInt do seu Cmd Mil A enquadrante</strong> ou o órgão ao qual o Sr. está subordinado.
                </p>
              </div>
            )}
            ...
            {janelaFechada && rascunhos.length > 0 && (
              <p className="text-sm text-zinc-500 mt-2">
                Você ainda tem {rascunhos.length} pedido{rascunhos.length !== 1 ? 's' : ''} em rascunho — use o botão <strong className="text-emerald-400">Enviar Pedidos</strong> acima para encaminhá-lo{rascunhos.length !== 1 ? 's' : ''} ao supervisor.
              </p>
            )}
          ...
          {janelaAtiva && dataFim && dataInicioJanela && (() => {
            ...
            return (
              <div className="h-1 bg-black/20">
                <div className={`h-full transition-all ${urgente ? 'bg-red-500' : 'bg-emerald-500'}`} style={{ width: `${pct}%` }} />
              </div>
            )
          })()}
        </div>
      ...
      {pedidos.length === 0 ? (
        ...
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handlePedidoDragEnd}>
