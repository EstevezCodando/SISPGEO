...
export function getStatusSolicitante(status: StatusPedido): StatusSolicitante {
  switch (status) {
    ...
    case 'ATRIBUIDO_CGEO':
    case 'APROVADO':
      return { label: 'Enviado', cls: 'bg-sky-500/10 text-sky-400 border border-sky-500/20' }
    case 'PRODUZIDO':
      return { label: 'Produto disponível', cls: 'bg-emerald-500/15 text-emerald-300 border border-emerald-500/30' }
    case 'REPROVADO':
      return { label: 'Inviável', cls: 'bg-red-500/10 text-red-400 border border-red-500/20' }
    case 'CANCELADO':
      return { label: 'Cancelado', cls: 'bg-zinc-800/50 text-zinc-500 border border-zinc-700/30' }
    default:
      return { label: status, cls: 'bg-zinc-700/40 text-zinc-400 border border-zinc-600/30' }
  }
}
